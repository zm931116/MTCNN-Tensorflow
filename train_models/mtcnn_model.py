...
def bbox_parameterization(bbox_pred, bbox_target, label):
    px = bbox_pred[:, 0]
    py = bbox_pred[:, 1]
    pw = bbox_pred[:, 2]
    ph = bbox_pred[:, 3]
    ...
    #tf.assert_rank(label,ones_index)
    # only keep positive and part samples
    valid_inds = tf.where(tf.equal(tf.abs(label), 1), ones_index, zeros_index)



    # paramiterize bounding boxes
    tx = tf.where(tf.equal(valid_inds, 1), (gx - px) / pw, zeros_index)
    ty = tf.where(tf.equal(valid_inds, 1), (gy - py) / ph, zeros_index)
    tw = tf.where(tf.equal(valid_inds, 1), tf.log(gw / pw), zeros_index)
    th = tf.where(tf.equal(valid_inds, 1), tf.log(gh / ph), zeros_index)

    parameterized_bbox = tf.stack([tx, ty, tw, th])
    print(parameterized_bbox.shape)
    pt = tf.transpose(parameterized_bbox)
    assert pt.shape == bbox_pred.shape
    print(pt.shape)
    return pt
...
def bbox_ohem_smooth_L1_loss(bbox_pred, bbox_target, label):
    ...
    #bbox_pred = bbox_parameterization(bbox_pred, bbox_target, label)
...
def P_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
    ...
    with slim.arg_scope([slim.conv2d],
        ...
        # tf.summary.image(name='layer1',tensor= net,max_outputs =3)
        _activation_summary(net)
        print(net.get_shape())
        ...
        if training:
            ...
            bbox_loss = bbox_ohem_smooth_L1_loss(bbox_pred, bbox_target, label)
            # batch*10
...
def R_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
    with slim.arg_scope([slim.conv2d],
       ...
       # _activation_summary(net)
        ...
        #_activation_summary(fc_flatten)
